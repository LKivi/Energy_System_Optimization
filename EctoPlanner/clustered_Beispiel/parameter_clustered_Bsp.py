...
#    weather_dict = weather_df.to_dict()
    ...
    dem["heat"] = clustered["heat"] / 1000000         # MW, heating load
    dem["cool"] = clustered["cool"] / 1000000         # MW, cooling load
    dem["power"] = clustered["power"] / 1000000          # MW, electrical load grid

    # Improve numeric by deleting very small loads
    eps = 0.01 # MW
    for load in ["power", "heat", "cool"]:
        for j in range(clustered["days"]):
            for k in range(clustered["time_steps"]):
                if dem[load][j][k] < eps:
                    dem[load][j][k] = 0
...
#             "pv_stc_area": 10000,          # m2,           roof area for pv or stc
    ...
    devs["BOI"] = {"inv_var": 52,       # kEUR/MW_th,       variable investment
                   ...
                   "min_load": 0.2,     # ---,              minimal partial load
                   "max_cap": 50,       # MW_th,            maximum heating capacity
                   "min_cap": 2,        # MW_th,            minimum heating capacity
                   }
    ...
    devs["CHP"] = {"inv_var": 570,      # kEUR/MW_el,       variable investment
                   ...
                   "min_load": 0.5,     # ---,              minimal partial load
                   "max_cap": 30,       # MW_th,            maximum heating capacity
                   "min_cap": 1,        # MW_th,            minimum heating capacity
                   }

...
#    dict_connections = par.cluster_connections(dict_clusters, edge_dict)
    ...
    total_costs = model.addVar(vtype="C", name="total_costs") # Objective function
    ...
    # objective
    model.addConstr(total_costs == sum( 0
                                        + x[edge] * param["inv_pipe_fix"][edge]
                                        + d_powered[edge] * param["inv_pipe_var"][edge]
                                        for edge in edges))
    
    # preset edges   
#    for con in connections:
#        model.addConstr(x[edge_dict[con]] == 1)
...
#                    print("Balancing power: t" + str(t) + ": " + str(round(m_bal[node,t].X,2)))
        ...
        print("Pipe caps and diameters:")
        for edge in edges:
            print("Edge " + str(edge_dict_rev[edge][0]) + "-" + str(edge_dict_rev[edge][1]) + ": " + str(round(cap[edge].X,2)) + " kg/s, " + str(round(d_powered[edge].X**0.4/100, 5)) + " m [x:" + str(abs(np.round(x[edge].X))) + "]")

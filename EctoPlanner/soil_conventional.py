...
#    T_soil = T_soil_clustered
 ...
 #%% HEATING GRID LOSSES  
    ...
    path = "input_data/pipes_steel.txt"
    ...
    for i in range(np.size(diameters)):
        pipe_geo[np.round(diameters[i],5)] = {"t_steel": t_steel[i],
                               "t_ins": t_ins[i],
                               "t_PE": t_PE[i]
                               }
    ...
    for p in pipe_data:
        d = pipe_data[p]["heat"]["diameter"]
        L = pipe_data[p]["heat"]["length"] 
        t1 = pipe_geo[d]["t_steel"]
        t2 = pipe_geo[d]["t_ins"]
        t3 = pipe_geo[d]["t_PE"]
        ...
        losses["heat"] = losses["heat"] + k*np.pi*d*L*((param["T_supply"]["heat"] - T_soil) + (param["T_return"]["heat"] - T_soil)) / 1e6
...
#%% COOLING LOSSES
    ...
    for p in pipe_data:
        d = pipe_data[p]["cool"]["diameter"]
        L = pipe_data[p]["cool"]["length"]  
        t = pipe_geo[d]["t_PE"]
        ...
        losses["cool"] = losses["cool"] + k*np.pi*d*L*((T_soil - param["T_supply"]["cool"]) + (T_soil - param["T_return"]["cool"]))  / 1e6 
...
def calculate_soil_temperature(param): 

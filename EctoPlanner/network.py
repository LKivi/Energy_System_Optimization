...
#    plt.show()   
    ...
    dir_BU = dir_network + "\\BU_placement"
...
#    pump_caps = model.addVars(pipes, vtype = "C", name = "pump_capacities")                            # W,        pump capacities
    ...
    tac_pipes = model.addVar(vtype = "C", name = "tac_pipes")                                           # EUR/a,   pipe annualized investment costs
    
    inv_pipes = model.addVar(vtype = "C", name = "pipe_inv_costs")                                      # EUR, pipe investment costs
    ...
    model.addConstr(inv_pipes >= sum((0.5*param["inv_earth_work"] + param["inv_pipe_PE"] * d_in[p] * d_in[p]) * 2*edge_lengths[p] for p in pipes), name = "Q1")
#    model.addConstr(inv_pipes <= sum((0.5*param["inv_earth_work"] + param["inv_pipe_PE"] * d_in[p] * d_in[p]) * 2*edge_lengths[p] for p in pipes), name = "Q2")
    ...
    model.addConstr(tac_pipes == inv_pipes * (param["ann_factor_pipe"] + param["cost_om_pipe"]))
...
#        print("Pipe " + str(edge_dict_rev[p][0]) + "-" + str(edge_dict_rev[p][1]) + ": " + str(round(d_in[p].X*10,2)) + " mm.")    
    ...
    for p in pipes:
        d_calc = d_in[p].X / 100
        d_min = ((8*dict_caps[p]**2*param["f_fric"])/(param["rho_f"]*np.pi**2*param["dp_pipe"]))**0.2
        for index in range(len(diameters)):
            if diameters[index] >= d_min:
                index_min = index
                break
        if d_calc < diameters[index_min]:
            d_norm = diameters[index_min]
        else:
            d_0 = diameters[index_min]
            d_1 = diameters[index_min+1]
            if abs(d_calc - d_0) < abs(d_calc - d_1):
                d_norm = d_0
            else:
                d_norm = d_1
        pipe_data[p] = {"diameter": d_norm,
    ...
    print("Pipe diameters:")
    for p in pipes:
        print("Pipe " + str(edge_dict_rev[p][0]) + "-" + str(edge_dict_rev[p][1]) + ": " + str(round(pipe_data[p]["diameter"]*1000,2)) + " mm.")
        
   
    # Recalculate pipe costs with norm diameters
    inv_pipes_norm = sum((param["inv_earth_work"] + 2 * param["inv_pipe_PE"] * (pipe_data[p]["diameter"]*100)**2) * edge_lengths[p] for p in pipes)
    tac_pipes_norm = inv_pipes_norm * (param["ann_factor_pipe"] + param["cost_om_pipe"]) / 1000
    
    
    # Recalculate annualized network costs
    tac_network_norm = tac_pipes_norm # + pump
    ...
    losses = soil.calculate_thermal_losses(param, pipe_data)
